addLayer("am", {
    ...
    getAMProd(){
        ...
        if (inChallenge('inf',31) && getBuyableAmount('am',22).gte(10)) prod = prod.mul(20)

        if (inChallenge('inf',42)) prod = prod.root(1.65).min(prod.div(1e6))
        return prod
    },
    getAMExp(){
        ...
        if (inChallenge('inf',12)) exp = exp.root(1.24).min(exp.div(1.5))

        if (exp.gte(9)) exp = exp.div(9).pow(0.5).mul(9)
        return exp
    },
    hotkeys:[
        {
    ...
    buyables:{
        11:{
            ...
            effect(x = getBuyableAmount(this.layer,this.id)){
                ...
                if (inChallenge('inf',22)) exp = exp.root(1.6).div(2)
                eff = eff.pow(exp)
                return eff
            },
        ...
        13:{
            ...
            effect(x = getBuyableAmount(this.layer,this.id)){
                ...
                let total = x.add(free)
                if (total.gte(52)) total = total.div(52).root(2).mul(52)
                let amount = total.mul(strength)
        ...
        22:{
            ...
            effect(x = getBuyableAmount(this.layer,this.id)){
                ...
                let amount = total.mul(strength)
                let exp = amount.pow(inChallenge('inf',31)?1:0.5)
                if (inChallenge('inf',31)) exp = exp.mul(-0.7)
        ...
        23:{
            ...
            effect(x = getBuyableAmount(this.layer,this.id)){
                ...
                let free = new Decimal(0)
                let total = x.add(free)
                if (total.gte(3)) total = total.div(3).root(2).mul(3)
                let amount = total.mul(strength)
